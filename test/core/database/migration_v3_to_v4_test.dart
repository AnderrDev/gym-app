  ...
  group('schema v4 (Phase 4 SWR polish)', () {
    test(
      ...
      () async {
        ...
        expect(
          tableNames,
          containsAll(['cached_assigned_routines', 'cached_weekly_insights']),
        );
    ...
    test(
      'PK compuesta de cached_assigned_routines: (user_id, routine_id)',
      () async {
        final db = openInMemoryDb();
        addTearDown(() async => db.close());
        await db.ping();
    ...
    test('PK compuesta de cached_weekly_insights: (user_id, routine_id, '
        'week_start)', () async {
      final db = openInMemoryDb();
      addTearDown(() async => db.close());
      await db.ping();
