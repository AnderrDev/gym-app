...
void main() {
  ...
  group('readRoutineDays / upsertRoutineDays', () {
    test('upsert reemplaza el contenido por routineId y devuelve por dayOfWeek',
        () async {
  ...
  group('replaceExercisesForDay / readExercisesForDay', () {
    test('join devuelve filas ordenadas por position con catálogo asociado',
        () async {
    ...
    test('replace borra los anteriores del día pero conserva catálogo global',
        () async {
      // 1ª escritura.
      await dao.replaceExercisesForDay(
        'd1',
  ...
  group('readLastPerformances / upsertLastPerformances', () {
    test('upsert por (userId, exerciseId) y read devuelve map por exerciseId',
        () async {
