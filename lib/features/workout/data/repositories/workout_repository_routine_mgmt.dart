...
mixin WorkoutRepositoryRoutineMgmtMixin on WorkoutRepository {
  ...
  Future<Either<Failure, Routine>> saveRoutine(Routine routine) =>
      guard(() async {
        final saved = await remoteDataSource.saveRoutine(
          RoutineModel.fromEntity(routine),
        );
  ...
  Future<Either<Failure, RoutineDay>> saveRoutineDay(RoutineDay day) =>
      guard(() async {
        final saved = await remoteDataSource.saveRoutineDay(
          RoutineDayModel.fromEntity(day),
        );
        return saved.toEntity();
      });
  ...
  @override
  Future<Either<Failure, Routine>> getRoutineById(String routineId) => guard(
    () async => (await remoteDataSource.getRoutineById(routineId)).toEntity(),
  );
