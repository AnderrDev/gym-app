        ...
        return BlocConsumer<WorkoutBloc, WorkoutState>(
          ...
          listener: (context, state) {
            if (state is ManagementSuccess) {
              ...
              if (!_pendingPop && mounted) {
                _pendingPop = true;
                context.pop(_hasMutations);
              }
            }
          ...
          builder: (context, state) {
            ...
            if (state is WeeklyPlanLoaded && !_isInitialized) {
              ...
              days = state.days;
              _isInitialized = true;
              _cachedDays = state.days;
            } else if (state is WeeklyPlanLoaded) {
              days = state.days;
              _cachedDays = state.days;
            } else if (_cachedDays.isNotEmpty) {
              days = _cachedDays;
            }
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                _hasMutations = true;
                workoutBloc.add(
                  DeleteRoutine(userId: userId, routineId: widget.routineId!),
                );
    ...
    return Container(
      ...
      child: GlassContainer(
        ...
        child: ListTile(
          ...
          onTap: () => context.push(
            AppRoutes.dayEditor,
            extra: {'day': day, 'routineId': widget.routineId ?? ''},
          ),
          leading: Container(
            width: 42,
            height: 42,
