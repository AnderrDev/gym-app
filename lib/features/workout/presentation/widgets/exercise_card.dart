...
class _ExerciseCardState extends State<ExerciseCard> with TickerProviderStateMixin {
  ...
  void _startRestTimer(int nextSetIndex) {
    ...
    _restTimer = Timer.periodic(const Duration(seconds: 1), (t) {
      ...
      if (_restSecondsLeft > 0 && _restSecondsLeft <= 3) HapticFeedback.lightImpact();
                    ...
                    children: [
                      Container(
                        ...
                        child: Icon(
                          ...
                          size: 20,
                          color: _allDone ? const Color(0xFF4CAF50) : AppColors.primary,
                        ),
                      ),
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(widget.exercise.name, style: AppTextStyles.bodyLarge.copyWith(fontWeight: FontWeight.bold)),
                              ...
                              children: [
                                Row(
                                  children: [
                                    ...
                                    Text('$_targetSets series', style: AppTextStyles.label.copyWith(color: AppColors.textSecondary)),
                                  ],
                                ),
                      ...
                      const SizedBox(width: 4),
                      if (widget.coachingAnalysis != null && widget.coachingAnalysis!.feedback != 'PENDING') ...[
                          ...
                          child: _isResting
                              ...
                              : Column(
                                  ...
                                  children: List.generate(_targetSets, (i) {
                                    final n = i + 1;
                                    return _SetRow(
                                      key: ValueKey('set_${widget.exercise.id}_$n'),
                                      setNumber: n,
                                      ...
                                      lastPerformanceLog: widget.lastPerformance,
                                      sessionId: widget.sessionId,
                                      exerciseId: widget.exercise.id,
                                      readOnly: widget.readOnly,
                                      onActivate: widget.readOnly ? () {} : () => _activateSet(n),
                                      onSaved: widget.readOnly ? (log) {} : (log) => _onSetSaved(log, n),
                                    );
                                  }),
                                ),
                        ),
  ...
  Widget _buildCoachingAdvice(CoachingAnalysis coaching, {bool compact = false}) {
    final score = coaching.performanceScore ?? 1.0;
    final isGood = score >= 0.85;
    final isGreat = score >= 1.0;
      ...
      return Container(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(
                coaching.recommendation.isEmpty ? (coaching.feedback ?? '') : _getFriendlyRecommendation(coaching.recommendation),
                style: AppTextStyles.label.copyWith(color: AppColors.textPrimary, fontStyle: FontStyle.italic),
                maxLines: 1,
...
class _SetRowState extends State<_SetRow> {
  ...
  void initState() {
    ...
    _weightCtrl = TextEditingController(text: initialWeight > 0 ? initialWeight.toStringAsFixed(0) : '');
    _repsCtrl = TextEditingController(text: initialReps > 0 ? initialReps.toString() : '');
  }

  @override
  void dispose() { _weightCtrl.dispose(); _repsCtrl.dispose(); super.dispose(); }
  ...
  Future<void> _save() async {
    ...
    final setLog = SetLog(sessionId: widget.sessionId, exerciseId: widget.exerciseId, actualWeight: weight, actualReps: reps, setIndex: widget.setNumber);
  ...
  Widget build(BuildContext context) {
      ...
      return InkWell(
        ...
        child: AnimatedContainer(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 12),
            Expanded(child: Text('${log.actualWeight} kg × ${log.actualReps} reps', style: AppTextStyles.bodyMedium.copyWith(fontWeight: FontWeight.w600))),
            if (!widget.readOnly)
              const Icon(Icons.edit, color: Color(0xFF4CAF50), size: 16, semanticLabel: 'Editar'),
          ]),
        ),
      );
    }
      ...
      return AnimatedContainer(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Text(widget.isDone ? 'Editando Serie ${widget.setNumber}' : 'Serie ${widget.setNumber}', style: AppTextStyles.bodyLarge.copyWith(color: AppColors.primary, fontWeight: FontWeight.bold)),
          ]),
          ...
          Row(children: [
                ...
                children: [
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      helperText: 'Objetivo: ${widget.targetWeight.toStringAsFixed(0)}kg',
                      helperStyle: const TextStyle(fontSize: 9, color: AppColors.textSecondary),
                      filled: true, 
            ...
            const SizedBox(width: 10),
            _saving ? const CircularProgressIndicator() : ElevatedButton(onPressed: _save, style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))), child: Icon(widget.isDone ? Icons.refresh : Icons.check, color: Colors.white)),
          ]),
        ]),
      );
    }
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(color: AppColors.background, borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFF2A2A2A))),
        child: Row(children: [
          _Circle(label: '${widget.setNumber}', filled: false),
          const SizedBox(width: 12),
          Text('Serie ${widget.setNumber} pendiente', style: AppTextStyles.label.copyWith(color: AppColors.textSecondary)),
        ]),
      ),
    );
  }
...
class _Circle extends StatelessWidget {
  ...
  final bool filled;
  final bool active;
  const _Circle({required this.label, required this.filled, this.active = false});
  @override build(BuildContext context) {
    final bg = filled ? const Color(0xFF4CAF50) : active ? AppColors.primary : Colors.transparent;
    final border = filled ? const Color(0xFF4CAF50) : active ? AppColors.primary : AppColors.textSecondary;
    final textColor = (filled || active) ? Colors.white : AppColors.textSecondary;
    return Container(width: 28, height: 28, decoration: BoxDecoration(color: bg, shape: BoxShape.circle, border: Border.all(color: border, width: 1.5)), child: Center(child: Text(label, style: TextStyle(color: textColor, fontSize: 13, fontWeight: FontWeight.bold))));
  }
}
